...
formNovoPaciente.addEventListener('submit', function (event) {
    ...
    fetch("https://localhost:8080/api/pacientes", {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
formNovoPaciente.addEventListener('submit', function (event) {
    ...
    fetch(`https://localhost:8080/api/pacientes?cpf=${novoPacienteData.cpf}`)
        .then(response => response.json())
        .then(data => {
            if (data.length > 0) {  // Verifica se algum paciente com o mesmo CPF foi encontrado
                ...
                return fetch("https://localhost:8080/api/pacientes", {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify(novoPacienteData),
                })
